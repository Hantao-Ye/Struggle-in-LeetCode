...
int main()
{
    ...
    if (in_txt.is_open() && out_txt.is_open())
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ListNode* head = read_list_node(in_txt);
            int k = read_int(in_txt);

            print_list_node(sol.reverseKGroup(head, k), out_txt);
