...
public:
    vector<int> findSubstring(string s, vector<string> &words)
    {
        ...
        int size = s.size(), len = words[0].size(), n = words.size();
        for (int i = 0; i < size - len * n + 1; i++)
        {
...
int main()
{
    ...
    if (in_txt.is_open() && out_txt.is_open())
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            vector<string> words = read_vector_string(in_txt);

            print_vector_int(sol.findSubstring(s, words), out_txt);
