...
private:
    ...
    void helper(string& digits, string temp_ans) {
        int index = temp_ans.size();
        if (index == digits.size()) {
            ans.push_back(temp_ans);
            return;
        }
        
        string choices = dict[digits[index] - '0' - 2];
        for (char c : choices)
        {
            helper(digits, temp_ans + c);
        }
    }
};
// @lc code=end
...
int main()
{
    ...
    if (in_txt.is_open() && out_txt.is_open())
    {
        ...
        for (int i = 0; i < n; i++)
        {
            string digits = read_string(in_txt);
            print_vector_string(s.letterCombinations(digits), out_txt);
