...
private:
    ...
    void helper(vector<int> &candidates, int target, vector<int> temp, int sum, int start)
    {
        if (sum > target)
        {
            return;
        ...
        {
            if (i > 0 && candidates[i] == candidates[i - 1] && i > start) // first hit in the sum
            {
                continue;
            }
...
int main()
{
    ...
    if (in_txt.is_open() && out_txt.is_open())
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            print_vector_vector_int(sol.combinationSum2(candidates, target), out_txt);
