...
public:
    void solveSudoku(vector<vector<char>> &board)
    {
        ...
        grid = vector<vector<int>>(9, vector<int>(10, 0));
        ...
        traversalSolveSudoku(board, 0, 0);
    }

private:
    vector<vector<int>> row, col, grid;

    bool traversalSolveSudoku(vector<vector<char>> &board, int i, int j)
    {
        if (i == 9)
            return true;
        else if (j == 9)
            return traversalSolveSudoku(board, i + 1, 0);
        else if (board[i][j] != '.')
            return traversalSolveSudoku(board, i, j + 1);
        ...
        for (int num = 1; num <= 9; num++)
        {
            if (feasible(board, i, j, num))
            {
                ...
                if (traversalSolveSudoku(board, i, j + 1))
                    return true;
                row[i][num]  = col[j][num] = grid[k][num] = 0;
                board[i][j] = '.';
            }
        }
    ...
    bool feasible(vector<vector<char>> &board, int i, int j, int num)
    {
        ...
        if (row[i][num] + col[j][num] + grid[k][num] == 0)
        {
            return true;
        }
...
int main()
{
    ...
    if (in_txt.is_open() && out_txt.is_open())
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            print_vector_vector_char(board, out_txt);
