...
private:
    vector<string> ans;
    void helper(string temp, int left_parent, int right_parent, int n)
    {
        if (left_parent == n && right_parent == n)
        {
            ans.push_back(temp);
            return;
        }

        if (left_parent < n || left_parent == right_parent)
        {
            helper(temp + '(', left_parent + 1, right_parent, n);
        }

        if (right_parent < left_parent)
        {
            helper(temp + ')', left_parent, right_parent + 1, n);
        }
    }
};
// @lc code=end
...
int main()
{
    ...
    if (in_txt.is_open() && out_txt.is_open())
    {
        ...
        for (int i = 0; i < n; i++)
        {
            int m = read_int(in_txt);
            print_vector_string(sol.generateParenthesis(m), out_txt);
